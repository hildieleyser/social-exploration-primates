...
# 5-Panel version with fixed A monkey and different male/female colors
# =============================================================================
...
# Color-blind safe palette for solo/duo/trio
cb_palette <- c("#0073C2FF", "#EFC000FF", "#868686FF", "#CD534CFF")

# DIFFERENT colors for male/female (distinct from solo/duo/trio)
sex_colors <- c("Male" = "#D55E00", "Female" = "#CC79A7")  # Orange and Pink

cat("=== CURRENT BIOLOGY 5-PANEL FIGURE (FIXED) ===\n")
...
cat("Monkeys in dataset:", paste(unique(data_clean$monkey), collapse = ", "), "\n")
...
# =============================================================================
# PANEL C: RANDOM-EFFECT SPREAD (FIXED TO INCLUDE ALL MONKEYS)
# =============================================================================

cat("Creating Panel C: Random-Effect Spread (FIXED)...\n")

# Extract random effects INCLUDING reference level
extract_random_effects_complete <- function(posterior_draws, data_clean) {
  random_data <- data.frame()
  
  # Get all unique monkeys in the dataset
  all_monkeys <- unique(data_clean$monkey)
  
  for(outcome in names(posterior_draws)) {
    for(monkey_name in all_monkeys) {
      # Convert to initial
      ...
      # Check if this monkey has a coefficient in the model
      term_name <- paste0("monkey_id", monkey_name)
      
      if(term_name %in% names(posterior_draws[[outcome]])) {
        # Has coefficient - use it
        draws <- posterior_draws[[outcome]][[term_name]]
        mean_val <- mean(draws)
        q025_val <- quantile(draws, 0.025)
        q975_val <- quantile(draws, 0.975)
      } else {
        # Reference level - effect is 0
        mean_val <- 0
        q025_val <- 0
        q975_val <- 0
      }
...
random_effects <- extract_random_effects_complete(posterior_draws, data_clean)
random_effects$monkey_ordered <- factor(random_effects$monkey, 
                                       levels = c("F", "D", "E", "C", "I", "A"))

cat("Random effects data includes monkeys:", paste(unique(random_effects$monkey), collapse = ", "), "\n")

pC <- ggplot(random_effects, aes(x = mean, y = monkey_ordered, color = sex)) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "grey50", linewidth = 0.4) +
  geom_pointrange(aes(xmin = q025, xmax = q975),
                  linewidth = 0.8, fatten = 3) +
  scale_color_manual(values = sex_colors, name = "Sex") +  # DIFFERENT COLORS
...
cat("Saving 5-panel Current Biology figure (FIXED)...\n")

# Save as PNG
ggsave("Figure2_CurrentBiology_5Panel_Fixed.png", final_figure_with_labels,
       width = CB_WIDTH_INCHES, height = CB_HEIGHT_INCHES, 
       dpi = 300, bg = "white")

# Save as PDF
ggsave("Figure2_CurrentBiology_5Panel_Fixed.pdf", final_figure_with_labels,
       width = CB_WIDTH_INCHES, height = CB_HEIGHT_INCHES, 
       dpi = 300, device = "pdf")
...
cat("\n5-PANEL CURRENT BIOLOGY FIGURE COMPLETE (FIXED)\n")
cat(paste(rep("=", 70), collapse = ""))

cat("\n\n✅ FIXES APPLIED:\n")
cat("- A monkey (ANEMONE) now included in Panel C as reference level (effect = 0)\n")
cat("- Male/Female colors changed to Orange/Pink (distinct from Solo/Duo/Trio)\n")
cat("- All 6 monkeys now properly displayed: F, D, E, C, I, A\n")

cat("\n📊 COLOR SCHEME:\n")
cat("- Solo/Duo/Trio: Blue/Yellow/Grey\n")
cat("- Male/Female: Orange/Pink (no conflict)\n")
cat("- Outcomes: Blue/Yellow/Grey (consistent throughout)\n")

cat("\n📁 FILES CREATED:\n")
cat("- Figure2_CurrentBiology_5Panel_Fixed.png\n")
cat("- Figure2_CurrentBiology_5Panel_Fixed.pdf\n")

# File sizes
png_size <- file.size("Figure2_CurrentBiology_5Panel_Fixed.png")
pdf_size <- file.size("Figure2_CurrentBiology_5Panel_Fixed.pdf")
cat(sprintf("\nFile sizes: PNG = %.1fKB, PDF = %.1fKB\n", 
            png_size/1024, pdf_size/1024))

cat("\n🎯 A MONKEY FOUND AND COLORS FIXED!\n")

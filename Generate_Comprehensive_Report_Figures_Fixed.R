# =============================================================================
# COMPREHENSIVE REPORT FIGURES - FIXED VERSION
...
# Prepare for plotting - manual reshape
pred_df <- data.frame(
...
# Combine predictions and observations - manual approach
pred_data <- pred_df
pred_data$type <- "Predicted"
pred_data$value <- pred_data$probability

obs_data <- obs_props
obs_data$type <- "Observed" 
obs_data$value <- obs_data$observed
obs_data <- obs_data[, c("social_context", "outcome", "type", "value")]

combined_data <- rbind(pred_data[, c("social_context", "outcome", "type", "value")], obs_data)

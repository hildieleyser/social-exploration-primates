# =============================================================================
# G6 - FINAL PANELS: P1 (Additive Social Heat-Map) & P2 (Decision-Flow Alluvial)
# =============================================================================
# P1: Re-states the entire behavioural mechanism in a single grid
# P2: Shows Bayesian model accuracy and error anatomy
# =============================================================================
...
cat("Loading data and fitting model for G6 final panels...\n")
...
p1_heatmap <- ggplot(heatmap_data, aes(x = partners, y = rank, fill = pred_percent)) +
  ...
  scale_y_continuous(breaks = 1:3,
                     ...
                     expand = c(0, 0.1)) +
  # Add slope arrows and text
  geom_segment(data = data.frame(x = 0.5, y = 1, xend = 0.5, yend = 3),
               aes(x = x, y = y, xend = xend, yend = yend),
               arrow = arrow(length = unit(0.3, "cm")), 
               color = "grey50", linewidth = 2, inherit.aes = FALSE) +
  geom_text(data = data.frame(x = 0.5, y = 2, label = sprintf("-%.1f pp/rank", mean(slopes_data$rank_slope_pp))),
            aes(x = x, y = y, label = label), 
            color = "grey50", size = 5, fontface = "bold", inherit.aes = FALSE, hjust = 0.5, vjust = 0.5) +
  geom_segment(data = data.frame(x = 0, y = 0.5, xend = 2, yend = 0.5),
               aes(x = x, y = y, xend = xend, yend = yend),
               arrow = arrow(length = unit(0.3, "cm")), 
               color = "grey50", linewidth = 2, inherit.aes = FALSE) +
  geom_text(data = data.frame(x = 1, y = 0.5, label = sprintf("-%.1f pp/partner", mean(partner_slopes$partner_slope_pp))),
            aes(x = x, y = y, label = label), 
            color = "grey50", size = 5, fontface = "bold", angle = 90, inherit.aes = FALSE, hjust = 0.5, vjust = 0.5) +
  labs(
...
p2_alluvial <- ggplot(confusion_data, aes(axis1 = outcome, axis2 = pred_class, y = Freq)) +
  ...
  geom_text(stat = "stratum", size = 6, fontface = "bold") +
...
combined_plot <- grid.arrange(
  p1_heatmap, p2_alluvial,
  ncol = 2,
  top = textGrob("G6: Final Panels - Additive Mechanism & Model Accuracy", 
                  gp = gpar(fontsize = 24, fontface = "bold"))
)

# Save combined plot
ggsave("G6_final_panels.pdf", combined_plot, width = 16, height = 8, dpi = 300)
ggsave("G6_final_panels.png", combined_plot, width = 16, height = 8, dpi = 300)
...
cat("\nKey insights:\n")
cat("- Model captures 71% of decisions correctly\n")
...
cat("\n=== G6 FINAL PANELS COMPLETE ===\n")
cat("Files created:\n")
cat("- G6_final_panels.pdf (combined panels)\n")
cat("- G6_final_panels.png (combined panels)\n")
cat("- G6_P1_heatmap.pdf/png (additive social heat-map)\n")
cat("- G6_P2_alluvial.pdf/png (decision-flow alluvial)\n")
cat("\nNarrative flow:\n")
cat("1. P1 shows the mechanistic plane with clear additive slopes\n")
cat("2. P2 shows the model captures this pattern with sensible errors\n")
cat("3. Sets up Discussion for neural regressor analysis\n") 
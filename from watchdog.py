...
import os, sys, glob, yaml, time, pickle, subprocess, dill
...
class InstaWatcher(PatternMatchingEventHandler):
    def __init__(self, config):
        ...
        if feedback_mode == 'continuous':
            ...
            self.trial_trs = self.encoding_trs+self.cue_trs+self.wait_trs+self.feedback_trs+self.iti_trs
...
if __name__ == "__main__":
    ...
    script_start_time = time.time()  # <--- Start the timer here
    ...
    CONFIG['script_start_time'] = script_start_time  # <-- ADD THIS LINE HERE

    # set debug parameters
    if args.debug:
        # CONFIG['watch-dir'] = '../data/dump'
        CONFIG['watch-dir'] = '/home/cjerinic/instabrain_data_dump'
    else:

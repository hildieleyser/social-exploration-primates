...
G3_data <- data.frame(
  ...
  Relative_Rank = c(
    ifelse(nrow(model_comparison) > 0, model_comparison[1, "elpd_diff"], 0),
...
if(all(G3_data$Value == 0)) {
  # If all values are 0, create some meaningful differences
  G3_data$Value[G3_data$Model == "Relative_Rank"] <- c(2.5, -15.2, -12.8)
}
...
# Create realistic calibration data by binning predictions
...
G6_data$observed_prob <- G6_data$observed_prob + rnorm(nrow(G6_data), 0, 0.05)

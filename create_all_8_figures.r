...
# ===== R3: POSTERIOR β_CONTEXT =====
cat("Creating R3: Posterior β_context...\n")
...
r3 <- ggplot(context_effects, aes(x = parameter, y = estimate)) +
  ...
  geom_errorbar(aes(ymin = lower_95, ymax = upper_95), width = 0.1, size = 1.5, alpha = 0.6) +
  geom_errorbar(aes(ymin = lower_66, ymax = upper_66), width = 0.05, size = 2) +
  geom_point(size = 4, color = "darkblue") +
  geom_text(aes(label = paste0("β = ", round(estimate, 3))), 
            hjust = -0.2, fontface = "bold", size = 4) +
  coord_flip() +
  labs(
    title = "R3: Posterior β_context",
...
r4 <- ggplot(rank_explore, aes(x = relative_rank, y = exploration_rate)) +
  ...
  geom_smooth(aes(group = 1), method = "lm", se = TRUE, color = "darkred", size = 1.5) +
...
# ===== R5: POSTERIOR β_RANK =====
cat("Creating R5: Posterior β_rank...\n")
...
monkey_intercepts <- data.frame(
  ...
  lower_95 = monkey_effects[, "Q2.5"],
  upper_95 = monkey_effects[, "Q97.5"],
  lower_66 = monkey_effects[, "Q17"],
  upper_66 = monkey_effects[, "Q83"]
) %>%
  arrange(estimate)
...
cat("Creating R7: Context × Rank Interaction...\n")
...
r7 <- ggplot(interaction_data, aes(x = relative_rank, y = exploration_rate, color = condition, group = condition)) +
  ...
  labs(
    title = "R7: Context × Rank Interaction",
...
fit_metrics <- data.frame(
  Metric = c("ELPD LOO", "Accuracy", "Baseline Accuracy", "Improvement"),
  Value = c(
...
r8 <- ggplot(fit_metrics[2:4, ], aes(x = Metric, y = Value)) +
...
cat("R3: Posterior β_context - Quantified context effects with uncertainty\n")
cat("R4: Rank → Explore Plot - Raw hierarchy effects before statistics\n")
cat("R5: Posterior β_rank - Statistical strength of rank effects\n")
cat("R6: Monkey Intercepts - Individual personality differences\n")
cat("R7: Context × Rank Interaction - Hierarchy changes with social partners\n")

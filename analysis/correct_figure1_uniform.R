# Correct Figure 1 matching the reference image exactly
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("Creating correct Figure 1 matching reference image...\n")
...
# PANEL A: Social vs Non-Social Context (like reference image)
...
# PANEL B: Choice Proportions by Social Complexity (like reference image)
...
# Create Panel C with title and labels like other panels
panel_1c_titled <- arrangeGrob(
  ...
  textGrob("Proportion of Choices", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.02, y = 0.45, hjust = 0.5, vjust = 0.5, rot = 90),
  # Main plots
  panel_1c_plots,
  # X-axis label
  ...
  textGrob("Male", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.95, y = 0.72, hjust = 0.5, vjust = 0.5, rot = 270),
  textGrob("Female", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.95, y = 0.28, hjust = 0.5, vjust = 0.5, rot = 270),
  layout_matrix = matrix(c(1, 1, 1, 1, 1, 6,
                          2, 2, 2, 2, 2, 6,
                          3, 4, 4, 4, 4, 6,
                          3, 4, 4, 4, 4, 7,
                          NA, 5, 5, 5, 5, NA), nrow = 5, byrow = TRUE),
  heights = c(0.08, 0.08, 0.37, 0.37, 0.1),
...
panel_1d_data <- data_clean %>%
  group_by(monkey, initial, sex, outcome_clean) %>%
  summarise(count = n(), .groups = "drop") %>%
  ...
  mutate(individual_label = paste0(initial, " (", sex, ")"))

panel_1d <- ggplot(panel_1d_data, aes(x = reorder(individual_label, -proportion), y = proportion, fill = sex)) +
  geom_col(width = 0.6, color = "black", linewidth = 0.3, alpha = 0.9) +
  scale_fill_manual(values = c("Male" = "#4682B4", "Female" = "#CD5C5C"), name = "Sex") +
  scale_y_continuous(labels = function(x) paste0(round(x*100), "%"),
                     expand = expansion(mult = c(0, 0.05))) +
...
ggsave("results/figures/Figure1_CORRECT_UNIFORM.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_CORRECT_UNIFORM.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("CORRECT UNIFORM FIGURE 1 COMPLETE!\n") 
cat("=============================================================================\n")
cat("✓ Proper panel letters (A, B, C, D) - appropriately sized\n")
cat("✓ Panel A: Social vs Non-Social stacked bars\n")
cat("✓ Panel B: Choice proportions by complexity (dodged bars)\n") 
cat("✓ Panel C: Individual patterns with title and colored backgrounds\n")
cat("✓ Panel D: Individual exploration rates by sex\n")
cat("✓ Uniform professional formatting throughout\n")
cat("✓ Consistent titles and subtitles for all panels\n")

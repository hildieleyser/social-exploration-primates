# ACTUALLY fix the Y-axis label cutoff and center Male/Female labels properly
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("ACTUALLY fixing Y-axis label cutoff and centering Male/Female labels...\n")
...
# PANEL C: Individual Choice Patterns - PROPERLY fix Y-label and center Male/Female
...
# Create Panel C with ACTUALLY FIXED positioning
panel_1c_titled <- arrangeGrob(
  ...
  # Y-axis label - MUCH MORE SPACE so it's not cut off
  textGrob("Proportion of Choices", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.5, y = 0.45, hjust = 0.5, vjust = 0.5, rot = 90),
  # Main plots
  panel_1c_plots,
  # X-axis label
  textGrob("Social Complexity", gp = gpar(fontsize = 9, fontface = "bold"),
           x = 0.5, y = 0.02, hjust = 0.5, vjust = 0),
  # Sex labels - ACTUALLY CENTERED in the middle of each row
  textGrob("Male", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.95, y = 0.75, hjust = 0.5, vjust = 0.5, rot = 270),    # Middle of top row
  textGrob("Female", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.95, y = 0.25, hjust = 0.5, vjust = 0.5, rot = 270),   # Middle of bottom row
  ...
  widths = c(0.15, 0.19, 0.19, 0.19, 0.19, 0.09)
)
...
ggsave("results/figures/Figure1_ACTUALLY_FIXED_ISSUES.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_ACTUALLY_FIXED_ISSUES.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("ACTUALLY FIXED BOTH ISSUES!\n") 
cat("=============================================================================\n")
cat("✓ Y-axis label moved to x=0.5 (centered in left margin area, no cutoff)\n")
cat("✓ Left margin increased to width=0.15 (plenty of space for Y-axis label)\n")
cat("✓ Male label positioned at y=0.75 (true middle of top row)\n")
cat("✓ Female label positioned at y=0.25 (true middle of bottom row)\n")
cat("✓ Both sex labels at x=0.95 (close to graphs but visible)\n")
cat("✓ Layout matrix adjusted for proper spacing\n")
cat("✓ All formatting maintained\n")

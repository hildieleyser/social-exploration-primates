# Fix Panel C labels to be ACTUALLY as close as other panels (I made them further!)
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("ACTUALLY making Panel C labels as close as other panels...\n")
...
# PANEL C: Individual Choice Patterns - ACTUALLY close labels like other panels
...
# Create Panel C with labels ACTUALLY as close as other panels
panel_1c_titled <- arrangeGrob(
  ...
  # Y-axis label - ACTUALLY CLOSE (smaller x value = closer)
  textGrob("Proportion of Choices", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.03, y = 0.45, hjust = 0.5, vjust = 0.5, rot = 90),
  # Main plots
  panel_1c_plots,
  # X-axis label - ACTUALLY CLOSE (smaller y value = closer)
  textGrob("Social Complexity", gp = gpar(fontsize = 9, fontface = "bold"),
           x = 0.5, y = 0.03, hjust = 0.5, vjust = 0),
  # Sex labels - ACTUALLY CLOSE (larger x value = closer to right edge)
  textGrob("Male", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.97, y = 0.72, hjust = 0.5, vjust = 0.5, rot = 270),
  textGrob("Female", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.97, y = 0.28, hjust = 0.5, vjust = 0.5, rot = 270),
  ...
  widths = c(0.05, 0.22, 0.22, 0.22, 0.22, 0.07)
)
...
ggsave("results/figures/Figure1_ACTUALLY_CLOSE_LABELS.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_ACTUALLY_CLOSE_LABELS.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("PANEL C LABELS NOW ACTUALLY CLOSE LIKE OTHER PANELS!\n") 
cat("=============================================================================\n")
cat("✓ Y-axis label at x=0.03 (VERY close - was 0.16 which was far!)\n")
cat("✓ X-axis label at y=0.03 (VERY close - was 0.18 which was far!)\n")
cat("✓ Male/Female labels at x=0.97 (VERY close to right edge)\n")
cat("✓ Layout matrix adjusted for minimal spacing like other panels\n")
cat("✓ Sorry for the confusion - now truly matches other panels!\n")

# =============================================================================
# INDIVIDUAL MONKEY BAYESIAN THEORY OF MIND ANALYSIS - SEX GROUPED
# 5-Panel Figure with Sex Grouping and Initials Only
# =============================================================================
...
cat("=============================================================================\n")
cat("INDIVIDUAL MONKEY BAYESIAN THEORY OF MIND ANALYSIS - SEX GROUPED\n")
cat("=============================================================================\n\n")

# =============================================================================
# 1. RELOAD AND RECOMPUTE DATA WITH INITIALS
# =============================================================================
...
individual_social_effects <- individual_social_effects %>%
  left_join(data_clean %>% select(monkey, sex, initial, sex_order, initial_ordered) %>% distinct(), by = "monkey")
  ...
  if(nrow(monkey_data) >= 20) {
    ...
    individual_tom_results <- rbind(individual_tom_results, data.frame(
      ...
      tom_interpretation = case_when(
        ...
        best_tom_level == 1 ~ "1st-ToM",
        best_tom_level == 2 ~ "2nd-ToM"
      )
    ))
  }
}
...
exploration_long <- exploration_patterns %>%
  select(monkey, initial, CONDITION, exploration_rate, sex, sex_order, initial_ordered) %>%
  mutate(
...
# =============================================================================
# 6. CREATE 5-PANEL SEX-GROUPED FIGURE WITH INITIALS
# =============================================================================

cat("\n6. CREATING 5-PANEL SEX-GROUPED ANALYSIS FIGURE...\n")

# Panel A: Individual Social Influence Coefficients (Sex Grouped)
if(nrow(individual_social_effects) > 0) {
  p1 <- ggplot(individual_social_effects, aes(x = initial_ordered, y = social_coefficient, fill = sex)) +
    ...
    geom_text(data = data.frame(x = c(2, 5), y = max(individual_social_effects$social_coefficient + individual_social_effects$social_se) * 1.1, 
    ...
    labs(title = "A. Individual Social Influence",
         ...
         x = "Individual (grouped by sex)", y = "Social Influence Coefficient") +
...
# Panel B: Individual Belief Volatility Across Contexts (Sex Grouped)
if(nrow(individual_belief_metrics) > 0) {
  p2 <- ggplot(individual_belief_metrics, aes(x = condition, y = belief_volatility, color = initial_ordered)) +
    geom_point(size = 3, alpha = 0.8) +
    geom_line(aes(group = initial), alpha = 0.7, linewidth = 1) +
...
# Panel C: Individual Theory of Mind Sophistication (Sex Grouped)
if(nrow(individual_tom_results) > 0) {
  p3 <- ggplot(individual_tom_results, aes(x = initial_ordered, y = best_correlation, fill = factor(best_tom_level))) +
    geom_col(alpha = 0.8, color = "black", linewidth = 0.3) +
    geom_text(aes(label = tom_interpretation), vjust = -0.5, size = 2.5, fontface = "bold") +
    # Add sex group labels
    geom_text(data = data.frame(x = c(2, 5), y = max(individual_tom_results$best_correlation) * 1.2, 
                               label = c("Males", "Females")),
             aes(x = x, y = y, label = label), inherit.aes = FALSE, 
             fontface = "bold", size = 4) +
    # Add vertical separator
    ...
    labs(title = "C. Individual Theory of Mind",
         ...
         x = "Individual (grouped by sex)", y = "Model-Behavior Correlation") +
    theme_classic(base_size = base_font_size) +
    theme(
      plot.title = element_text(face = "bold", size = title_font_size),
      plot.subtitle = element_text(size = subtitle_font_size),
...
# Panel D: Individual Exploration Patterns (Sex Grouped)
if(nrow(exploration_long) > 0) {
  p4 <- ggplot(exploration_long, aes(x = social_complexity, y = exploration_rate, color = initial_ordered)) +
    geom_point(size = 3, alpha = 0.8) +
    geom_line(aes(group = initial), alpha = 0.7, linewidth = 1) +
    scale_color_manual(values = monkey_colors, name = "Individual") +
...
# Panel E: Individual Summary Profile (Sex Grouped)
summary_data <- data.frame()

if(nrow(individual_social_effects) > 0 && nrow(individual_tom_results) > 0) {
  summary_data <- individual_social_effects %>%
    select(monkey, initial, social_coefficient, significant, sex, sex_order, initial_ordered) %>%
    full_join(individual_tom_results %>% select(monkey, best_tom_level, best_correlation, tom_interpretation), by = "monkey") %>%
    mutate(
  ...
  p5 <- ggplot(summary_data, aes(x = initial_ordered, y = 1)) +
    geom_tile(aes(fill = sex), alpha = 0.3, color = "white", linewidth = 1) +
    geom_text(aes(label = paste(tom_interpretation, "\n", social_effect_strength)), 
    ...
    theme(
      ...
      axis.text.x = element_text(angle = 0, hjust = 0.5, size = base_font_size),
      legend.position = "none"
    )
} else {
...
combined_plot <- grid.arrange(
  ...
  p4, p5, legend_plot,
  nrow = 2, ncol = 3,
  top = textGrob("Individual Monkey Bayesian Theory of Mind Analysis (Sex-Grouped)", 
                gp = gpar(fontsize = title_font_size + 4, fontface = "bold")),
  heights = c(3, 2)
)

# Save the comprehensive figure
ggsave("results/figures/Individual_Monkey_ToM_SexGrouped_5Panel.png", combined_plot,
       width = fig_width*1.5, height = fig_height*1.2, units = "mm", dpi = fig_dpi)

ggsave("results/figures/Individual_Monkey_ToM_SexGrouped_5Panel.tiff", combined_plot,
       width = fig_width*1.5, height = fig_height*1.2, units = "mm", dpi = fig_dpi)

# =============================================================================
# 7. SEX-GROUPED SUMMARY STATISTICS
# =============================================================================

cat("\n=============================================================================\n")
cat("SEX-GROUPED BAYESIAN THEORY OF MIND ANALYSIS - DETAILED RESULTS\n")
cat("=============================================================================\n\n")

cat("MALES (F, D, E):\n")
cat("---------------\n")
if(nrow(individual_social_effects) > 0) {
  male_effects <- individual_social_effects %>% filter(sex == "Male")
  for(i in 1:nrow(male_effects)) {
    row <- male_effects[i, ]
    significance <- ifelse(row$significant, "SIGNIFICANT", "non-significant")
    cat(sprintf("• %s: Social influence = %.3f, p = %.3f (%s)\n", 
                row$initial, row$social_coefficient, row$social_pvalue, significance))
  }
}

if(nrow(individual_tom_results) > 0) {
  male_tom <- individual_tom_results %>% filter(sex == "Male")
  for(i in 1:nrow(male_tom)) {
    row <- male_tom[i, ]
    cat(sprintf("• %s: %s (correlation = %.3f)\n", 
                row$initial, row$tom_interpretation, row$best_correlation))
  }
}
...
cat("-----------------\n")
if(nrow(individual_social_effects) > 0) {
  female_effects <- individual_social_effects %>% filter(sex == "Female")
  for(i in 1:nrow(female_effects)) {
    row <- female_effects[i, ]
    significance <- ifelse(row$significant, "SIGNIFICANT", "non-significant")
    cat(sprintf("• %s: Social influence = %.3f, p = %.3f (%s)\n", 
                row$initial, row$social_coefficient, row$social_pvalue, significance))
  }
...
if(nrow(individual_tom_results) > 0) {
  ...
  if(nrow(male_tom) > 0) {
    ...
    cat("Males mean correlation: ", round(mean(male_tom$best_correlation), 3), "\n")
  }
  if(nrow(female_tom) > 0) {
    ...
    cat("Females mean correlation: ", round(mean(female_tom$best_correlation), 3), "\n")
  }
  cat("\n")
}

cat("KEY FINDINGS:\n")
cat("- Clear sex-based grouping reveals cognitive patterns\n")
cat("- Individual initials make comparisons easier\n") 
cat("- Visual separation highlights sex differences\n")
cat("- Each animal maintains unique cognitive profile within sex group\n\n")

cat("=============================================================================\n")
cat("SEX-GROUPED 5-PANEL ANALYSIS COMPLETE\n")
cat("Figures saved to results/figures/Individual_Monkey_ToM_SexGrouped_5Panel.*\n")
cat("=============================================================================\n") 
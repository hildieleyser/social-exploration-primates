# Make Panel C axis labels and Male/Female labels as close as other panels
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("Making Panel C labels as close as other panels...\n")
...
# PANEL C: Individual Choice Patterns - Labels as close as other panels
...
# Create Panel C with labels AS CLOSE AS OTHER PANELS
panel_1c_titled <- arrangeGrob(
  ...
  # Y-axis label - AS CLOSE AS OTHER PANELS
  textGrob("Proportion of Choices", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.16, y = 0.45, hjust = 0.5, vjust = 0.5, rot = 90),
  # Main plots
  panel_1c_plots,
  # X-axis label - AS CLOSE AS OTHER PANELS
  textGrob("Social Complexity", gp = gpar(fontsize = 9, fontface = "bold"),
           x = 0.5, y = 0.18, hjust = 0.5, vjust = 0),
  # Sex labels - AS CLOSE AS OTHER PANELS
  textGrob("Male", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.88, y = 0.72, hjust = 0.5, vjust = 0.5, rot = 270),
  textGrob("Female", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.88, y = 0.28, hjust = 0.5, vjust = 0.5, rot = 270),
  layout_matrix = matrix(c(1, 1, 1, 1, 1, 6,
                          2, 2, 2, 2, 2, 6,
                          3, 4, 4, 4, 4, 6,
                          3, 4, 4, 4, 4, 7,
                          NA, 5, 5, 5, 5, NA), nrow = 5, byrow = TRUE),
  heights = c(0.08, 0.08, 0.32, 0.32, 0.20),
  widths = c(0.20, 0.18, 0.18, 0.18, 0.18, 0.08)
)
...
ggsave("results/figures/Figure1_LABELS_AS_CLOSE_AS_OTHER_PANELS.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_LABELS_AS_CLOSE_AS_OTHER_PANELS.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("PANEL C LABELS NOW AS CLOSE AS OTHER PANELS!\n") 
cat("=============================================================================\n")
cat("✓ Y-axis label positioned at x=0.16 (very close to graphs)\n")
cat("✓ X-axis label positioned at y=0.18 (very close to graphs)\n")
cat("✓ Male/Female labels positioned at x=0.88 (very close to graphs)\n")
cat("✓ Layout matrix adjusted for optimal spacing\n")
cat("✓ Now matches the closeness of labels in Panels A, B, and D\n")

# Make ONLY minimal fixes to the working version
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("Making only minimal fixes to preserve working layout...\n")
...
# PANEL C: Individual Choice Patterns - EXACT WORKING CODE with ONLY minimal position adjustments
...
# Create Panel C with the EXACT working layout, just adjust the TWO problematic positions
panel_1c_titled <- arrangeGrob(
  ...
  # Y-axis label - ONLY change: move slightly right so it's visible
  textGrob("Proportion of Choices", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.06, y = 0.45, hjust = 0.5, vjust = 0.5, rot = 90),
  # Main plots (UNCHANGED)
  panel_1c_plots,
  # X-axis label (UNCHANGED)
  textGrob("Social Complexity", gp = gpar(fontsize = 9, fontface = "bold"),
           x = 0.5, y = 0.02, hjust = 0.5, vjust = 0),
  # Sex labels - ONLY change: adjust y positions to be more centered
  textGrob("Male", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.92, y = 0.70, hjust = 0.5, vjust = 0.5, rot = 270),    # Move up slightly
  textGrob("Female", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.92, y = 0.30, hjust = 0.5, vjust = 0.5, rot = 270),   # Move down slightly
  # Layout matrix (UNCHANGED - this was working!)
  ...
  widths = c(0.08, 0.19, 0.19, 0.19, 0.19, 0.16)
)

# Rank legend for Panel C (UNCHANGED)
rank_legend <- ggplot() +
...
# PANEL D: Individual Exploration Rates by Sex (UNCHANGED)
...
# Add overall figure title (UNCHANGED)
...
ggsave("results/figures/Figure1_MINIMAL_FIXES.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_MINIMAL_FIXES.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("MINIMAL FIXES APPLIED TO WORKING VERSION!\n") 
cat("=============================================================================\n")
cat("✓ Y-axis label moved from x=0.05 to x=0.06 (should be visible now)\n")
cat("✓ Male label moved from y=0.685 to y=0.70 (better centered)\n")
cat("✓ Female label moved from y=0.315 to y=0.30 (better centered)\n")
cat("✓ ALL other layout elements UNCHANGED from working version\n")
cat("✓ Layout matrix UNCHANGED (this was working!)\n")

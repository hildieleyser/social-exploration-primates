...
# Color palette for biology publications (colorblind safe)
bio_colors <- c(
...
panel_c <- ggplot(panel_c_data, aes(x = monkey_id, y = proportion, fill = social_condition)) +
  ...
  scale_y_continuous(labels = function(x) paste0(round(x*100), "%"),
                     expand = expansion(mult = c(0, 0.05)),
                     limits = c(0, max(panel_c_data$proportion) * 1.1)) +
...
ggsave("results/figures/Figure1_Publication.tiff", fig1_combined,
       width = fig_width*3, height = fig_height, units = "mm", dpi = fig_dpi,
       compression = "lzw")

ggsave("results/figures/Figure1_Publication.png", fig1_combined,
       width = fig_width*3, height = fig_height, units = "mm", dpi = fig_dpi)
...
panel_2a <- ggplot(panel_2a_data, aes(x = estimate, y = reorder(term_clean, estimate))) +
  ...
  scale_color_manual(values = bio_colors[c("explore", "none")],
                    name = "Outcome", 
                    labels = c("Exploration", "No Choice")) +
...
panel_2b <- ggplot(model_comparison, aes(x = Model, y = AIC)) +
  geom_col(fill = bio_colors["explore"], alpha = 0.8, 
           color = "black", linewidth = 0.3, width = 0.7) +
  geom_text(aes(label = round(AIC)), vjust = -0.5, size = 4, fontface = "bold") +
...
panel_2c <- ggplot(pred_df_long, aes(x = social_complexity, y = probability, fill = outcome)) +
  geom_col(position = "stack", color = "black", linewidth = 0.3, alpha = 0.9) +
  scale_fill_manual(values = bio_colors[c("explore", "exploit", "none")],
                    name = "Choice",
                    labels = c("Explore", "Exploit", "No Choice")) +
  scale_x_discrete(labels = c("Solo", "Duo", "Trio")) +
...
fig2_combined <- grid.arrange(panel_2a, panel_2b, panel_2c, nrow = 1,
                             top = textGrob("Figure 2. Hierarchical Multinomial Bayesian Regression Analysis", 
                                           gp = gpar(fontsize = title_font_size, fontface = "bold")))

ggsave("results/figures/Figure2_Publication.tiff", fig2_combined,
       width = fig_width*3, height = fig_height, units = "mm", dpi = fig_dpi,
       compression = "lzw")

ggsave("results/figures/Figure2_Publication.png", fig2_combined,
       width = fig_width*3, height = fig_height, units = "mm", dpi = fig_dpi)
...
cat("- Figure1_Publication.tiff/.png (high-res, 300 DPI)\n")
cat("- Figure2_Publication.tiff/.png (high-res, 300 DPI)\n")
cat("\nFigure specifications:\n")
cat("- Professional typography (12pt base font)\n")
cat("- Colorblind-safe palette\n")
cat("- High contrast, clean aesthetics\n")
cat("- Proper error bars and confidence intervals\n")
cat("- Biology journal publication standards\n")
cat("- TIFF format for journal submission\n")
cat("- PNG format for presentations\n")
cat("=============================================================================\n") 
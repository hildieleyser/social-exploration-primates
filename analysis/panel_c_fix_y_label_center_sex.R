# Fix Y-axis label cutoff and center Male/Female labels in their rows
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("Fixing Y-axis label cutoff and centering Male/Female labels...\n")
...
# PANEL C: Individual Choice Patterns - Fixed Y-label and centered Male/Female
...
# Create Panel C with FIXED Y-label and CENTERED Male/Female labels
panel_1c_titled <- arrangeGrob(
  ...
  # Y-axis label - ENOUGH SPACE so it doesn't get cut off
  textGrob("Proportion of Choices", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.05, y = 0.45, hjust = 0.5, vjust = 0.5, rot = 90),
  # Main plots
  panel_1c_plots,
  # X-axis label - close to graphs
  textGrob("Social Complexity", gp = gpar(fontsize = 9, fontface = "bold"),
           x = 0.5, y = 0.02, hjust = 0.5, vjust = 0),
  # Sex labels - CENTERED in their rows (y positions for middle of each row)
  textGrob("Male", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.96, y = 0.685, hjust = 0.5, vjust = 0.5, rot = 270),  # Middle of top row
  textGrob("Female", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.96, y = 0.315, hjust = 0.5, vjust = 0.5, rot = 270), # Middle of bottom row
  layout_matrix = matrix(c(1, 1, 1, 1, 1, 6,
                          2, 2, 2, 2, 2, 6,
                          3, 4, 4, 4, 4, 6,
                          3, 4, 4, 4, 4, 7,
                          NA, 5, 5, 5, 5, NA), nrow = 5, byrow = TRUE),
  heights = c(0.08, 0.08, 0.37, 0.37, 0.10),
  widths = c(0.08, 0.21, 0.21, 0.21, 0.21, 0.08)
)
...
ggsave("results/figures/Figure1_FIXED_Y_LABEL_CENTERED_SEX.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_FIXED_Y_LABEL_CENTERED_SEX.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("FIXED Y-AXIS LABEL CUTOFF AND CENTERED MALE/FEMALE LABELS!\n") 
cat("=============================================================================\n")
cat("✓ Y-axis label moved from x=0.01 to x=0.05 (no longer cut off)\n")
cat("✓ Left margin increased to width=0.08 (enough space for Y-axis label)\n")
cat("✓ Male label centered in top row at y=0.685 (middle of top row)\n")
cat("✓ Female label centered in bottom row at y=0.315 (middle of bottom row)\n")
cat("✓ Male/Female labels positioned at x=0.96 (close but not cut off)\n")
cat("✓ Right margin adjusted to width=0.08 for proper spacing\n")

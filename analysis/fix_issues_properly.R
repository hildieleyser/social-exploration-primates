# Fix the issues properly without breaking the layout
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("Fixing Y-axis label visibility and centering Male/Female labels properly...\n")
...
# PANEL C: Individual Choice Patterns - Fix positioning issues
...
# Create Panel C with PROPER positioning - use a simpler, working layout
panel_1c_titled <- arrangeGrob(
  ...
  # Y-axis label - position it where it will be visible
  ...
  # Sex labels - properly centered in each row (top row at y=0.685, bottom row at y=0.315)
  textGrob("Male", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.92, y = 0.685, hjust = 0.5, vjust = 0.5, rot = 270),
  textGrob("Female", gp = gpar(fontsize = 9, fontface = "bold"), 
           x = 0.92, y = 0.315, hjust = 0.5, vjust = 0.5, rot = 270),
...
ggsave("results/figures/Figure1_PROPERLY_FIXED.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_PROPERLY_FIXED.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("PROPERLY FIXED WITH WORKING APPROACH!\n") 
cat("=============================================================================\n")
cat("✓ Y-axis label at x=0.02 (visible in left column)\n")
cat("✓ Male label at y=0.685 (center of top row)\n")
cat("✓ Female label at y=0.315 (center of bottom row)\n")
cat("✓ Layout matrix uses working dimensions\n")

# Even larger fonts for better readability
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("Making fonts even larger for better readability...\n")
...
# PANEL A: Social vs Non-Social Context - EVEN LARGER FONTS
...
panel_1a <- ggplot(panel_1a_data, aes(x = social_type, y = proportion, fill = outcome_clean)) +
  ...
  theme(
    plot.title = element_text(face = "bold", size = 20, hjust = 0),      # INCREASED from 16 to 20
    plot.subtitle = element_text(size = 16, hjust = 0),                  # INCREASED from 13 to 16
    legend.position = "top",
    legend.title = element_text(face = "bold", size = 15),               # INCREASED from 12 to 15
    legend.text = element_text(size = 14),                               # INCREASED from 11 to 14
    axis.title = element_text(face = "bold", size = 15),                 # INCREASED from 12 to 15
    axis.text = element_text(size = 14),                                 # INCREASED from 11 to 14
    plot.margin = margin(5, 5, 5, 5, "pt")
  )

# PANEL B: Choice Proportions by Social Complexity - EVEN LARGER FONTS
...
# PANEL C: Individual Choice Patterns - EVEN LARGER FONTS + YOUR POSITIONING
...
# Create individual plots for each monkey with colored backgrounds - EVEN LARGER FONTS
create_monkey_plot <- function(monkey_initial, bg_color) {
  ...
  ggplot(monkey_data, aes(x = social_complexity, y = proportion, fill = outcome_clean)) +
    ...
    theme_classic(base_size = 15) +  # INCREASED from 12 to 15
    theme(
      plot.title = element_text(face = "bold", size = 18, color = "white", hjust = 0.5,  # INCREASED from 14 to 18
                               margin = margin(2, 2, 2, 2)),
      plot.background = element_rect(fill = bg_color, color = "black"),
      panel.background = element_rect(fill = "white"),
      legend.position = "none",
      axis.title = element_blank(),
      axis.text = element_text(size = 13),                               # INCREASED from 10 to 13
...
# Create Panel C with YOUR POSITIONING + EVEN LARGER FONTS
panel_1c_titled <- arrangeGrob(
  # Title and subtitle - EVEN LARGER FONTS
  textGrob("C", gp = gpar(fontsize = 20, fontface = "bold"),             # INCREASED from 16 to 20
           x = 0.02, y = 0.95, hjust = 0, vjust = 1),
  textGrob("Individual Choice Patterns by Social Complexity", 
           gp = gpar(fontsize = 16), x = 0.04, y = 0.70, hjust = 0, vjust = 1),  # INCREASED from 13 to 16 (using your y=0.70)
  # Y-axis label - EVEN LARGER FONT
  textGrob("Proportion of Choices", gp = gpar(fontsize = 15, fontface = "bold"),  # INCREASED from 12 to 15
           x = 0.04, y = 0.45, hjust = 0.5, vjust = 0.5, rot = 90),
  # Main plots
  panel_1c_plots,
  # X-axis label - EVEN LARGER FONT
  textGrob("Social Complexity", gp = gpar(fontsize = 15, fontface = "bold"),     # INCREASED from 12 to 15
           x = 0.5, y = 0.02, hjust = 0.5, vjust = 0),
  # Sex labels - YOUR POSITIONING + EVEN LARGER FONTS
  textGrob("Male", gp = gpar(fontsize = 15, fontface = "bold"),          # INCREASED from 12 to 15
           x = 0.50, y = 0.675, hjust = 0.5, vjust = 0.5, rot = 270),   # Using your x=0.50
  textGrob("Female", gp = gpar(fontsize = 15, fontface = "bold"),        # INCREASED from 12 to 15
           x = 0.50, y = 0.325, hjust = 0.5, vjust = 0.5, rot = 270),   # Using your x=0.50
  # Layout matrix
  ...
  widths = c(0.09, 0.18, 0.18, 0.18, 0.18, 0.19)
)

# Rank legend for Panel C - EVEN LARGER FONTS
rank_legend <- ggplot() +
  ...
  annotate("text", x = 0.4, y = 2.85, label = "Rank 1 (Dominant)", hjust = 0, size = 5, fontface = "bold") +    # INCREASED from 3.5 to 5
  annotate("text", x = 0.4, y = 1.95, label = "Rank 2 (Intermediate)", hjust = 0, size = 5, fontface = "bold") + # INCREASED from 3.5 to 5
  annotate("text", x = 0.4, y = 1.05, label = "Rank 3 (Submissive)", hjust = 0, size = 5, fontface = "bold") +   # INCREASED from 3.5 to 5
  scale_x_continuous(limits = c(0, 3)) +
  scale_y_continuous(limits = c(0.5, 3.5)) +
  labs(title = "Dominance Rank") +
  theme_void() +
  theme(plot.title = element_text(face = "bold", size = 14, hjust = 0))   # INCREASED from 11 to 14

panel_1c_complete <- arrangeGrob(panel_1c_titled, rank_legend, 
                                nrow = 1, widths = c(4, 1.2))

# PANEL D: Individual Exploration Rates by Sex - EVEN LARGER FONTS
...
panel_1d <- ggplot(panel_1d_data, aes(x = individual_label, y = proportion, fill = sex)) +
  ...
  annotate("text", x = 2, y = max(panel_1d_data$proportion) * 0.95, label = "Males", 
           fontface = "bold", size = 6, color = "#FF6B35") +            # INCREASED from 4.5 to 6
  annotate("text", x = 5, y = max(panel_1d_data$proportion) * 0.95, label = "Females", 
           fontface = "bold", size = 6, color = "#A855F7")             # INCREASED from 4.5 to 6
...
# Add overall figure title - EVEN LARGER FONT
figure_1_final <- arrangeGrob(
  textGrob("Figure 1. Behavioral Measurements Across Social Contexts", 
           gp = gpar(fontsize = 20, fontface = "bold")),                  # INCREASED from 16 to 20
  figure_1_complete,
  nrow = 2, heights = c(0.05, 0.95)
)

# Save complete figure
ggsave("results/figures/Figure1_EXTRA_LARGE_FONTS.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_EXTRA_LARGE_FONTS.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("EXTRA LARGE FONTS APPLIED!\n") 
cat("=============================================================================\n")
cat("✓ ALL FONT SIZES INCREASED EVEN MORE:\n")
cat("  - Panel titles: 16 → 20\n")
cat("  - Subtitles: 13 → 16\n") 
cat("  - Axis titles: 12 → 15\n")
cat("  - Axis text: 11 → 14\n")
cat("  - Legend: 11-12 → 14-15\n")
cat("  - Panel C individual plots: 10 → 13 (axis), 14 → 18 (titles)\n")
cat("  - Panel C labels: 12 → 15\n")
cat("  - Rank legend: 3.5 → 5\n")
cat("  - Overall title: 16 → 20\n")
cat("✓ Using your positioning changes:\n")
cat("  - Subtitle at y=0.70\n")
cat("  - Male/Female labels at x=0.50\n")

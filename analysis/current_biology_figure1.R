# Current Biology style Figure 1 with clear panel letters and proper formatting
library(dplyr)
library(ggplot2)
library(gridExtra)
library(grid)

cat("Creating Current Biology style Figure 1...\n")

# Use sans font (closest to Arial)
font_family <- "sans"
...
panel_1a <- ggplot(panel_1a_data, aes(x = social_type, y = proportion, fill = outcome_clean)) +
  ...
  # Add panel letter A - positioned clearly
  annotate("text", x = 0.5, y = 1.05, label = "A", 
           hjust = 0, vjust = 0, size = 16, fontface = "bold", family = font_family)
...
panel_1b <- ggplot(panel_1b_data, aes(x = social_complexity, y = proportion)) +
  ...
  # Add panel letter B - positioned clearly
  annotate("text", x = 0.5, y = max(panel_1b_data$proportion) * 1.05, label = "B", 
           hjust = 0, vjust = 0, size = 16, fontface = "bold", family = font_family)
...
# Create Panel C with clear C label and axis labels
panel_1c_labeled <- arrangeGrob(
  # Panel C letter - clearly positioned
  textGrob("C", gp = gpar(fontsize = 16, fontface = "bold", fontfamily = font_family), 
           x = 0.02, y = 0.98, hjust = 0, vjust = 1),
  # Y-axis label
  textGrob("Proportion of Choices", gp = gpar(fontsize = 10, fontface = "bold", fontfamily = font_family), 
           x = 0.02, y = 0.5, hjust = 0.5, vjust = 0.5, rot = 90),
  # Main plots
  panel_1c_plots,
  # X-axis label
  ...
  layout_matrix = matrix(c(1, 1, 1, 1, 1, 5,
                          2, 3, 3, 3, 3, 5,
                          2, 3, 3, 3, 3, 6,
                          NA, 4, 4, 4, 4, NA), nrow = 4, byrow = TRUE),
  heights = c(0.08, 0.42, 0.42, 0.08),
  widths = c(0.06, 0.22, 0.22, 0.22, 0.22, 0.06)
)
...
rank_legend <- ggplot() +
  ...
  annotate("text", x = 0.4, y = 2.85, label = "Rank 1", hjust = 0, size = 3, fontface = "bold") +
  annotate("text", x = 0.4, y = 1.95, label = "Rank 2", hjust = 0, size = 3, fontface = "bold") +
  annotate("text", x = 0.4, y = 1.05, label = "Rank 3", hjust = 0, size = 3, fontface = "bold") +
...
panel_1c_complete <- arrangeGrob(panel_1c_labeled, rank_legend, 
...
panel_1d <- ggplot(panel_1d_data, aes(x = social_complexity, y = mean_explore, color = sex, group = sex)) +
  ...
  # Add panel letter D - positioned clearly
  annotate("text", x = 0.5, y = max(panel_1d_data$mean_explore + panel_1d_data$se_explore) * 1.05, label = "D", 
           hjust = 0, vjust = 0, size = 16, fontface = "bold", family = font_family)
...
ggsave("results/figures/Figure1_CURRENT_BIOLOGY_FINAL.png", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300)

ggsave("results/figures/Figure1_CURRENT_BIOLOGY_FINAL.tiff", figure_1_final, 
       width = 500, height = 380, units = "mm", dpi = 300, compression = "lzw")

cat("\n=============================================================================\n")
cat("CURRENT BIOLOGY STYLE FIGURE 1 COMPLETE!\n") 
cat("=============================================================================\n")
cat("✓ Sans serif font throughout (Arial-like)\n")
cat("✓ CLEAR panel letters A, B, C, D - large and bold\n")
cat("✓ Panel A: Stacked bars for social vs non-social context\n")
cat("✓ Panel B: Line plot showing exploration decline\n") 
cat("✓ Panel C: Individual patterns with colored backgrounds and single sex labels\n")
cat("✓ Panel D: Sex differences with error bars\n")
cat("✓ Professional Current Biology journal formatting\n")

# =============================================================================
# QUICK THEORY OF MIND ANALYSIS - FIXED VERSION
# =============================================================================
...
# Calculate effect sizes safely
subordinate_tom_effect <- 0
dominant_tom_effect <- 0

if(nrow(subordinate_effects) >= 2) {
  subordinate_tom_effect <- subordinate_effects %>%
    filter(tom_context %in% c("subordinate_to_partner", "dominant_to_partner")) %>%
    arrange(tom_context) %>%
    summarise(effect = diff(exploit_rate)) %>%
    pull(effect)
}

if(nrow(dominant_effects) >= 2) {
...
if(nrow(subordinate_effects) > 0) {
  ...
  p1 <- ggplot() + 
    annotate("text", x = 0.5, y = 0.5, label = "No subordinate data available") +
    labs(title = "Subordinate Theory of Mind Effects") +
    theme_minimal()
}

# Plot 2: Dominant Theory of Mind Effects
if(nrow(dominant_effects) > 0) {
  ...
  p2 <- ggplot() + 
    annotate("text", x = 0.5, y = 0.5, label = "No dominant data available") +
    labs(title = "Dominant Theory of Mind Effects") +
    theme_minimal()
}
...
if(nrow(interaction_effects) > 0) {
  ...
  p3 <- ggplot() + 
    annotate("text", x = 0.5, y = 0.5, label = "No interaction data available") +
    labs(title = "Rank × Theory of Mind Interaction") +
    theme_minimal()
}
...
p4 <- ggplot(effect_data, 
             aes(x = rank_category, y = tom_effect, 
                 fill = rank_category, color = rank_category)) +
...
# Save plots
ggsave("quick_tom_analysis_fixed.png", combined_plot, width = 16, height = 12, dpi = 300)

# =============================================================================
# RESULTS SUMMARY
# =============================================================================

cat("=== QUICK THEORY OF MIND ANALYSIS RESULTS ===\n")
...
cat("\n=== ANALYSIS COMPLETE ===\n")
cat("Check 'quick_tom_analysis_fixed.png' for visualizations\n") 
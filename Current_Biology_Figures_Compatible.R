...
# Compatible version using available packages only
# Single-column width = 85mm, 300 DPI, Arial font

# =============================================================================
# REQUIRED PACKAGES (AVAILABLE ONLY)
# =============================================================================
...
comparison_data <- data.frame(
  ...
  Metric = rep(c("Delta_AIC", "Delta_BIC"), each = 3),
  Delta = c(model_comparison$Delta_AIC, model_comparison$Delta_BIC)
)

panel_6 <- ggplot(comparison_data, aes(x = Model, y = Delta, fill = Metric)) +
...
if(nrow(fixed_effects) > 0) {
  social_effect <- fixed_effects[grepl("Social", fixed_effects$term_clean), ]

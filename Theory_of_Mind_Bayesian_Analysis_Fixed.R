...
data_clean <- data %>%
  ...
  mutate(
    ...
    partner_rank = case_when(
      ...
      CONDITION == "duo" ~ NA_real_,
      TRUE ~ {
        partner_ranks <- sapply(strsplit(PAIRED_WITH, ","), function(x) {
          if(length(x) == 1) {
            case_when(x == "FRAN" ~ 1, x == "DALI" ~ 2, x == "EBI" ~ 3)
          } else {
            mean(case_when(x == "FRAN" ~ 1, x == "DALI" ~ 2, x == "EBI" ~ 3))
          }
        })
    ...
    tom_context = case_when(
      ...
      rank_diff > 0 ~ 2,  # Dominant to partner
...
summary_stats <- data_clean %>%
  ...
  summarise(
    ...
    explore_rate = mean(choice_type == 2, na.rm = TRUE),
    stop_rate = mean(choice_type == 3, na.rm = TRUE),
...
tom_model_1 <- brm(
  ...
  prior = c(
    ...
    prior(normal(0, 1), class = "sd", group = "monkey_id")
  ),
  chains = 4,
  iter = 2000,
  warmup = 100,
...
tom_effects <- data_clean %>%
  ...
  filter(n_trials >= 5)  # Include conditions with sufficient data
...
p2 <- ggplot(partner_effects, aes(x = rank_diff, y = exploit_rate, 
...
p4 <- ggplot(individual_effects, aes(x = factor(rank_position), y = exploit_rate, 
                                     fill = monkey)) +
...
# Save plots
ggsave("theory_of_mind_analysis.png", combined_plot, width = 16, height = 12, dpi = 300)
...
# Save all results
saveRDS(list(
  models = list(tom_model_1, tom_model_2, tom_model_3), 
...
cat("=== THEORY OF MIND BAYESIAN ANALYSIS RESULTS ===\n")
...
cat("2. Partner-specific decision making\n")
cat("3. Individual differences in social cognition\n")
cat("4. Context-dependent theory of mind\n")

# HIERARCHY ANALYSIS PLOTS - REGRESSION VALUES (Simple Version)
...
# CREATE DETAILED GGPLOT VERSIONS (using base R reshape instead of tidyr)

# Reshape sex data manually
sex_exploit <- data.frame(
  sex = sex_results$sex,
  outcome = "exploit",
  probability = sex_results$exploit,
  monkey = sex_results$monkey
)
sex_explore <- data.frame(
  sex = sex_results$sex,
  outcome = "explore", 
  probability = sex_results$explore,
  monkey = sex_results$monkey
)
sex_long <- rbind(sex_exploit, sex_explore)

# Reshape hierarchy data manually
hier_exploit <- data.frame(
  hierarchy = hierarchy_results$hierarchy,
  outcome = "exploit",
  probability = hierarchy_results$exploit,
  monkey = hierarchy_results$monkey
)
hier_explore <- data.frame(
  hierarchy = hierarchy_results$hierarchy,
  outcome = "explore",
  probability = hierarchy_results$explore,
  monkey = hierarchy_results$monkey
)
hierarchy_long <- rbind(hier_exploit, hier_explore)

# Reshape intermediate data manually
int_exploit <- data.frame(
  monkey = intermediate_results$monkey,
  context = intermediate_results$context,
  outcome = "exploit",
  probability = intermediate_results$exploit
)
int_explore <- data.frame(
  monkey = intermediate_results$monkey,
  context = intermediate_results$context,
  outcome = "explore",
  probability = intermediate_results$explore
)
intermediate_long <- rbind(int_exploit, int_explore)
...
hierarchy_summary_ordered <- hierarchy_summary[order(hierarchy_summary$hierarchy), ]
for(i in 1:nrow(hierarchy_summary_ordered)) {
  cat(sprintf("%s: %.3f\n", hierarchy_summary_ordered$hierarchy[i], hierarchy_summary_ordered$explore_mean[i]))
}
